...
void publish_message(MQTT::Client<MQTTNetwork, Countdown>* client) { //PC/Python
  if (mode == 1) {
      ...
      char buff[100];
      sprintf(buff, "Gesture Event #%d: , ID: %d", IDEvents, curGesture);
      ...
      if (IDEvents > 4) {
        if (feature0 > feature1) {
          if (feature0 > feature2) {
            result = 0;
          }
        } else if (feature1 > feature0) {
  ...
  if (mode == 2) {
      MQTT::Message message;
      char buff[100];
      sprintf(buff, "The extracted features: %d", result);
      message.qos = MQTT::QOS0;
      message.retained = false;
      message.dup = false;
      message.payload = (void*) buff;
...
int main(int argc, char* argv[]) {
  ...
  while (true) {
    ...
    while(mode == 1) {
        ...
        //X[indexValue] = tiltValue[0];
        //Y[indexValue] = tiltValue[1];
        //Z[indexValue] = tiltValue[2];
        //indexValue++;
        ...
        if (gesture_index < label_num) {
          ...
          if (curGesture == 0) {
            feature0++;
          } else if (curGesture == 1) {
            feature1++;
          } else if (curGesture == 2) {
            feature2++;
          }
          error_reporter->Report(config.output_message[curGesture]);
    ...
    while (mode == 2) {
      ...
      mqtt_queue2.call(&publish_message, &client);
      printf("The classified gesture events: \n");
      error_reporter->Report(config.output_message[result]);
...
void angleSelection() {
    ...
    while(mode == 1) {
      if (curGesture == 0) {
        uLCD.locate(3, 6);
        uLCD.printf("gesture ID: %d", curGesture);
        //feature0++;
        //uLCD.printf("X value: %d", reference[0]);
        ...
        uLCD.printf("gesture ID: %d", curGesture);
        //feature1++;
        //uLCD.printf("Y value: %d", reference[1]);
        ...
        uLCD.printf("gesture ID: %d", curGesture);
        //feature2++;
        //uLCD.printf("Z value: %d", reference[2]);
